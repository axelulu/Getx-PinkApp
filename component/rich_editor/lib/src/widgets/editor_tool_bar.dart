...
class EditorToolBar extends StatelessWidget {
            ...
            child: ListView(
              ...
              children: [
                ...
                TabButton(
                  ...
                  icon: Icons.image,
                  onTap: () async {
                    List<AssetEntity>? fileList = await getImage(context);
                    for (int i = 0; i < fileList!.length; i++) {
                      File? file = await fileList[i].file;
                      String url = "";
                      if (getImageUrl != null && await fileList[i].exists) {
                        url = await getImageUrl!(file!);
                        await javascriptExecutor.insertImage(
                          url,
                          alt: fileList[i].title,
                        );
                      }
                    }
                  },
  ...
  Future<List<AssetEntity>?> getImage(context) async {
